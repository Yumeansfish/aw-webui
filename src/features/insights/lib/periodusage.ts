...
import { seconds_to_duration, get_hour_offset } from '~/app/lib/time.ts';
import {
  PERIOD_USAGE_BAR,
  PERIOD_USAGE_BAR_FOCUSED,
  PERIOD_USAGE_BAR_SELECTED,
  ...
  _.each(usage_arr, (events, i: number) => {
    ...
    if (moment(date).isSame(moment(), 'day')) {
      svg
        ...
        .attr('x2', x + width / 2 + '%')
        .attr('y2', 200)
        .attr('style', `stroke: ${PERIOD_USAGE_TODAY_LINE}; stroke-width: 2px;`)
