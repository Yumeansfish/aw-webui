...
import {
  ACTIVITY_TIMESPIRAL_ACTIVE,
  ACTIVITY_TIMESPIRAL_IDLE,
  ACTIVITY_TIMESPIRAL_INNER_STROKE,
  ACTIVITY_TIMESPIRAL_LABEL,
  ACTIVITY_TIMESPIRAL_TEXT,
  ACTIVITY_TIMESPIRAL_TRACK,
    ...
    renderChart() {
      ...
      events.forEach((e: IEvent & { startAngle: number; endAngle: number }, i) => {
        ...
        */
        ...
        g.append('g')
          ...
          .attr('fill', e.data.status == 'not-afk' ? ACTIVITY_TIMESPIRAL_ACTIVE : ACTIVITY_TIMESPIRAL_IDLE)
          .attr('opacity', 0.7);
      });

      const labelColor = ACTIVITY_TIMESPIRAL_LABEL;
      const tickColor = ACTIVITY_TIMESPIRAL_IDLE;
      ...
      function drawClockTick(group, a, radius, inner = false) {
        ...
        if (inner)
          group
            ...
            .attr('x2', (radius - 5) * xn)
            .attr('y2', (radius - 5) * yn)
            .style('stroke', ACTIVITY_TIMESPIRAL_INNER_STROKE)
            .style('stroke-width', 2);
      }
      ...
      function drawDate(group, date) {
        ...
        group
          ...
          .style('fill', 'none')
          .style('stroke', ACTIVITY_TIMESPIRAL_TRACK)
        ...
        group
          ...
          .style('fill', ACTIVITY_TIMESPIRAL_TEXT);
      }
