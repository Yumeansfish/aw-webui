...
import {
  ACTIVITY_SUNBURST_LABEL,
  ACTIVITY_SUNBURST_TICK,
  ACTIVITY_SUNBURST_TRAIL_LABEL,
...
function drawClockTick(a: number) {
  ...
  vis
    ...
    .attr('x2', 155 * xn)
    .attr('y2', 155 * yn)
    .style('stroke', ACTIVITY_SUNBURST_TICK)
...
function drawClock(h: number, m: number, text?: string) {
  ...
  vis
    ...
    .style('fill', ACTIVITY_SUNBURST_LABEL)
    .attr('x', 130 * xn)
    .attr('y', 5 + 140 * yn);
}
...
function initializeBreadcrumbTrail() {
  ...
  trail.append('svg:text').attr('id', 'endlabel').style('fill', ACTIVITY_SUNBURST_TRAIL_LABEL);
}
